...
- 支持观察 `left4dead2.exe` 网络流量，从真实 TCP/UDP 包中提取 L4D2 服务器 IPv4。
- 观察模式会解析 Steam master 返回包和 A2S_INFO 包，尽量保留游戏列表里出现过但当前不响应的服务器。
- 结果区使用标准 Win32 表格控件显示，避免伪表格错位。
...
1. Flow 层识别 `left4dead2.exe` 使用的本地 TCP/UDP 端口。
2. Network 层监听真实 IPv4 TCP/UDP 包，并只处理属于游戏端口的流量。
3. 从 Steam master 返回包中拆出服务器 `IPv4:Port`。
4. 从 A2S_INFO 响应包中直接解析服务器名、地图、人数和标签。
5. 对候选服务器再次执行 A2S_INFO 查询，尽量验证服务器当前是否真的可响应。

这比单纯读取 WinDivert Flow 远端地址更可靠，因为 L4D2 刷新服务器列表时可能使用一个本地 UDP socket 与大量服务器通信，Flow 层远端地址可能出现 `0.0.255.255:*` 这类通配/非真实服务器地址。

如果游戏列表包里能解析出服务器名称，但工具主动 A2S 查询时服务器不响应，结果区会在命中原因中标记 `A2S未响应`。如果只能拿到候选地址，且延迟、人数、地图都未知，这类未知候选会被标记为 `命中`，导出 IP、导出火绒和应用防火墙时都会包含它们。

观察模式不会自动屏蔽任何 IP。只有点击 `应用屏蔽` 或导出后手动导入火绒，才会进入真正阻断流程。

## 结果区

结果区使用标准表格控件，字段包括：状态、延迟、人数、地图、地址、服务器、命中原因。

状态含义：

- `正常`：未命中当前筛选规则。
- `命中`：服务器名命中左侧关键词。
- `未响应`：游戏包里出现过，但工具主动 A2S 查询时没有响应。
...
左侧文本框中每行一个服务器名关键词。普通服务器的命中状态只按服务器名匹配，不会因为地图、标签、Ping、人数、密码或 IP 规则被标成 `命中`。只有“候选服务器/未获得名称服务器且延迟、人数、地图全未知”的条目会被额外视为命中，方便后续屏蔽。工具会对服务器名做简单标准化后匹配：
